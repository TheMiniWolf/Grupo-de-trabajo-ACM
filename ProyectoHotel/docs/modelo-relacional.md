# Modelo Relacional ΓÇö Sistema de Gesti├│n Hotelera

Diagrama entidad-relaci├│n de la base de datos `proyecto_hotel`. Refleja la
migraci├│n Flyway `V1__crear_tablas.sql` y las restricciones a├▒adidas en
...
| `reserva` Γåö `habitacion` | N : M | v├¡a `reserva_habitacion` | Reserva de varias habitaciones |
| `reserva` Γåö `servicio` | N : M | v├¡a `reserva_servicio` | Servicios adicionales por reserva |
| `reserva` | 1 : 1 | `pago` | Un pago por reserva (`uq_pago_reserva`) |
| `pago` | 1 : 1 | `factura` | Una factura por pago (`uq_factura_pago`) |

## Normalizaci├│n y restricciones

- **3FN**: cat├ílogos separados (`rol`, `tipo_habitacion`, `servicio`) y tablas
...
- **Dominios controlados**: restricciones `CHECK` que reflejan los enums de la
  aplicaci├│n (estados, m├⌐todos de pago, tipos de servicio).
- **Reglas de negocio**: `fecha_salida > fecha_entrada`, montos y cantidades
  no negativos.
- **├ìndices** sobre todas las claves for├íneas para escalabilidad de consultas.
